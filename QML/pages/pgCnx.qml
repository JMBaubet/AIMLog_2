...
Rectangle {
    ...
    Label {
        id: titre
        text: qsTr("Suppression et importation des fichiers connection.")
...
Si vous voulez importer des nouvelles données dans la base, vous devez télécharger au péalable vos fichiers dans le répertoire : <br><br>    - <b>" +
    ...
    Item {
        ...
        Button {
            ...
            onClicked: backend.importCnxFile(myNbFile, myDomaine)
        }
    }
    ...
    Dialog {
        ...
        modal: true
        title: myNbFile === 1 ? "Suppression du fichier connexion*.zip sélectionné ?" : "Suppression des "+ myNbFile + " fichiers connexion*.zip sélectionnés ?"
        standardButtons: MessageDialog.No | MessageDialog.Yes
        visible: false
        onAccepted: backend.delCnxFile(myNbFile)
    ...
    Connections{
        ...
        function onSetNbFileCnx(nbFile) {
            myNbFile = nbFile
            if (nbFile === 0) {
